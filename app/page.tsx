...
export default function Home() {
  ...
  async function fetchQuestions(page: number = 1) {
    const res = await fetch(`/api/questions?_page=${page}&order_by=${orderBy}`);
    if (!res.ok) {
  ...
  const handlePageChanging = (event: BaseSyntheticEvent) => {
    redirect(
      `http://localhost:3000?page=${event.target.innerText}&order_by=${orderBy}`,
    );
  };

  const handleOrderByChanging = (event: BaseSyntheticEvent) => {
    redirect(
      `http://localhost:3000?page=${currentPage}&order_by=${event.target.value}`,
    );
  };
